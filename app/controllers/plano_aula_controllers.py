...
def criar_plano():
    ...
    campos_obrigatorios = ['titulo', 'objetivo', 'ementa', 'data_prevista', 'disciplina']
...
def listar_planos():
    filtros = {
        ...
        "ordenar_por": request.args.get('ordenar_por', default='data_prevista', type=str)
    }
...
def editar_plano(id):
    ...
    try:
        ...
        return jsonify({
            "mensagem": f"Plano {id} atualizado com sucesso!",
            "dados": resultado
        }), 200
    except ValueError as ve:
        return jsonify({"erro": str(ve)}), 404
...
def gerar_recomendacoes_ia():
    ...
    if not dados or 'titulo' not in dados or 'disciplina' not in dados or 'ementa' not in dados:
        return jsonify({"erro": "Para gerar recomendações, informe Título, Disciplina e Ementa/Resumo."}), 400
    ...
    try:
        ...
        return jsonify({"erro": f"Falha na comunicação com o assistente de IA: {str(e)}"}), 504