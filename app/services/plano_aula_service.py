...
class PlanoAulaService:
    ...
    def criar_plano(dados):
        ...
        logging.info(f"Plano de Aula '{novo_plano.titulo}' salvo com sucesso no banco de dados.")
    ...
    def gerar_recomendacoes_ia(titulo, disciplina, ementa):
        ...
        resposta_ia, tokens_usados = AIService.gerar_sugestoes(titulo, disciplina, ementa)
        
        latencia = round(time.time() - inicio, 2)
        
        logging.info(f"AI Request: Title=\"{titulo}\", Discipline=\"{disciplina}\", Token Usage={tokens_usados}, Latency={latencia}s.")
